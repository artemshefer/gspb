...
my $paramLine = shift @ARGV;

my $path_of_this_module = File::Basename::dirname( eval { ( caller() )[1] } );
if ($path_of_this_module eq '.'){ $path_of_this_module = '..'; }
my $pathToCfg = '../conf/config.txt';

my $params = getConfig($pathToCfg);

if ($paramLine && $paramLine eq 'check'){
    if (keys %{$params}){
        print "\nCheck connect: it works.\n";
    }
    exit 1;
}
...
############################################
# create a list of incoming files
############################################
sub getListOfFiles($$){
    my $path    = shift;
    my $relPath = shift;
...
;
    ...
    if ($value->{"flag"} && $value->{"flag"} eq '<='){
        ...
        if ($ref->{numrows} eq '0E0'){
            ...
            ($ref->{numrows}, my $db_err) = markDunplicates(
                $dbh
              , $file
              , $line
              , $value->{"date"}
...
############################################
# save duplicates into 'tech_log'
############################################
sub markDunplicates($$$$$$){
my ($dbh, $file, $line, $date, $str, $tbl_dpl, $operation) = @_;
...
;
    ...
    my $ref->{numrows} = $sth_t->execute(
            ...
            $file, $tbl_dpl, $operation,
            'Duplicate, line#: '.$line.'; '.$date.' '.$str
    );

    return ($ref->{numrows}, $dbh->err);
}
...
sub getSetDataSQL($$$){
    ...
    if ($typeOfQuery eq 'select'){
        ...
        my ($slctdFields)     = $sql =~ m/^select[\s]+(.+?)[\s]+from/i; 
        $ref->{fieldNames}    = [split ",", $slctdFields];
    }
    # if($dbh->err){
